...
public static class MyExtensions
{
    ...
    public static string AggregateAsHumanReadable<T>(
        this IEnumerable<T> items,
        Func<T, string> selector,
        byte maxNumberOfItems,
        string separator = ", ",
        string finalConjunction = "and",
        string singleItemWord = "other",
        string pluralItemWord = "others")
    {
        var groupings = items
            .Select((x, i) => new
            {
                Item = selector(x),
                GroupingKey = i < maxNumberOfItems - 1
                    ? -1
                    : i < maxNumberOfItems
                        ? 0
                        : 1
            })
            .GroupBy(x => x.GroupingKey, x => x.Item)
            .ToList();
        
        switch (groupings.Count)
        {
            case 0:
                return string.Empty;
        }
        
        IList<string> head = groupings.Take(2).SelectMany(g => g).ToList();
        int tailCount = groupings.Skip(2).SelectMany(g => g).Count();
        
        if (head.Count == 1)
            return head.Single();
            
        if (head.Count == 2)
            return string.Format("{0} {1} {2}", head.First(), finalConjunction.Trim(), head.Last());
            
        if (tailCount == 0)
            return string.Join(separator, head.Take(head.Count - 1)) + separator + finalConjunction.Trim() + " " + head.Last();
...
public class CouchbaseHelper
{
    ...
    public IEnumerable<string> GetAllDocumentKeys(string bucketName)
    {
        var cluster = new Cluster();
        cluster.Configuration.Servers = new List<Uri>() { new Uri(new Uri(Host), new Uri("/pools", UriKind.Relative)) };
        ...
        using (var bucket = cluster.OpenBucket(bucketName, ""))
        {
            var query = bucket.CreateQuery("release", "all_documents");
            query.Limit(int.MaxValue);
            
            return bucket.Query<dynamic>(query).Rows.Select(r => r.Id);
        }
    }
